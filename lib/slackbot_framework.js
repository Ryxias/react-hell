...
// Configuration
const SLACK_BOT_TOKEN = 'xoxb-235188197473-o4GOUJ31DVDjLMHIr6NWDreW'; // Probably a bad idea to keep in the public domain but we can re-generate it
const CHUUBOT_USER_ID = 'U6X5J5TDX';
...
class Chuubot {
    ...
    this.rtm.on(RTM_EVENTS.MESSAGE, (message) => {
      ...
      for (let listener of this.listeners) {
        ...
        if (listener.matches(message)) {
          this.log('Match');
          this.rtm.sendMessage(listener.callback(message), message.channel);
          return;
        }
        this.log('Pass');
      }
   ...
   * - callback: Accepts a single argument, message.  Returns the string Slackbot should output on match.
   *
   * @param listener
   */
...
class ChuubotListenerRegexp {
  ...
  matches(message) {
    let message_text = message.text;
    console.log(message_text.match(this.regex));
    return null !== message_text.match(this.regex);
  }

  callback(message) {
    return this.callback_function(message);
  }
}


const chuu = new Chuubot(SLACK_BOT_TOKEN, CHUUBOT_USER_ID);
chuu.connect();
