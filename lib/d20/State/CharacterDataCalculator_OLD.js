...
class CharacterDataCalculator {
  ...
  calculateHealth() {
    ...
    const current_calculations = {
      'compute.health.max': this.compute.health.max.stat,
  ...
  calculateAttackBonuses() {
    const base_attack_bonus = combineMatrix(this.data.base_attack_bonus);
    const ranged_calculations = {
      base_attack: base_attack_bonus,
      dexterity: this.compute.ability_modifiers.dexterity.stat,
      size: 0, // FIXME(derek)
    };
    this.compute.attack_bonuses.ranged = statCalculation(ranged_calculations);

    const melee_calculations = {
      base_attack: base_attack_bonus,
      strength: this.compute.ability_modifiers.strength.stat,
      size: 0, // FIXME(derek)
    };
    this.compute.attack_bonuses.melee = statCalculation(melee_calculations);
  }
}

/**
 * Returns the { stat, calculation } tuple for any calculated stat
 *
 * @param matrix
 * @param ignore
 * @returns {{stat, calculation: *}}
 */
