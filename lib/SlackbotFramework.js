...
module.exports = (slack_config) => {
  ...
  class Chuubot {
    ...
    /**
     * @returns {Promise} A Document object
     */
    getChuubotConfig() {
      return Document.findOne({ where: { name: 'chuu-config' } });
    }

    /**
     * @returns {Promise} A new Document model
     */
    initializeChuubotConfig() {
     ...
     * @returns {Promise}
     */
    registerPrivateChannelMapping(user_id, private_channel_id) {
      return this.getChuubotConfig()
        .then(chuu_config => {
          if (!chuu_config) {
            return this.initializeChuubotConfig();
          }
          return chuu_config;
        })
        .then(chuu_config => {
    ...
    /**
     * @return {Promise} The configured private slack channel id, or undefined
     */
    getPrivateMessageChannel(user_id) {
      return Document.findOne({ where: { name: 'chuu-config' }})
        .then(chuu_config => {
          if (!chuu_config) {
