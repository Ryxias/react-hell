...
class Slackbot {
  constructor(bot_token, bot_user_id, verbose = false) {
    ...
    let base_listener = {};
    base_listener.handles = (message) => {
      let message_text = message.text();
      return message_text.toUpperCase() === '<@' + this.user_id + '> ARE YOU THERE?';
    };
    base_listener.run = (message, output) => {
      output.reply("Hello <@" + message.getUser() + ">, I am here!");
    };
    this.addScript(base_listener);
  }
