...
class Slackbot {
  ...
  setupRtmConnection() {
    ...
    this.rtm.on('message', this.receiveMessage);
  }

  connect() {
    this.setupRtmConnection();
    this.rtm.start();
    this.webclient = new WebClient(this.bot_token);
  }
  ...
  sendMessage(message_text, channel_id) {
    ...
    //   text: message_text,
    // };
    // return this.rtm.addOutgoingEvent(false, message.type, message)
    //   .then((resp) => this.log('Successfully sent message back:', resp))
    //   .catch(console.error);


    // https://api.slack.com/methods/chat.postMessage
    return this.webclient.chat.postMessage(
      { channel: channel_id, text: message_text, as_user: this.user_id, }
    );
  }

  addReaction(reaction_name, original_message) {

    // https://api.slack.com/methods/reactions.add
    return this.webclient.reactions.add(
      {
        channel: original_message.getChannel(),
        name: reaction_name,
        as_user: this.user_id,
        timestamp: original_message.getTimestamp(),
      }
    );
  }
