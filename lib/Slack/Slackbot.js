...
class Slackbot {
  constructor(bot_token, bot_user_id, verbose = false) {
    ...
    this.rtm.on(RTM_EVENTS.MESSAGE, slack_message => {
      // FIXME (derek)
      //   As a future enhancement, instead of attaching a complex listener right here,
      //   we should have THIS code construct an event and dispatch it (containing
      //   the message as well as the output buffer).
      //
      //   Then the scripts should listen for Events. This allows systems other than
      //   RTM to listen for "Events" and to control the Slackbot behavior.
