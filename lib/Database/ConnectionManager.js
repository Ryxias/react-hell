...
class ConnectionManager {
  ...
  constructor(mysql_config, is_production = true, ServiceContainer) {
  ...
  loadModels() {
    ...
    model_files.forEach(filename => {
      ...
      this.models[model_name] = require(model_path)(this.ServiceContainer, this.sequelize_connection);
    });
  }
  ...
  // load associations
  loadAssociations() {
    const SlackUser = this.get('SlackUser');
    const User = this.get('User');
    SlackUser.belongsTo(User, {foreignKey: 'user_id'}); // Adds user_id fk to SlackUser

    const RockPaperScissorsPlayer = this.get('RockPaperScissorsPlayer');
    const RockPaperScissorsGame = this.get('RockPaperScissorsGame');

    RockPaperScissorsPlayer.belongsTo(RockPaperScissorsGame, {foreignKey: 'game_id'});
    RockPaperScissorsPlayer.belongsTo(User, {foreignKey: 'user_id'});
  }
