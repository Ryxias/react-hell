...
class Player {
  ...
  joinGame() {
    return this.engine.addPlayer(this)
      .then(() => {
        this.output.notifyPlayerJoined(this.player_id);
      })
  ...
  stay() {
    return this.isMyTurn()
      ...
      .then(() => this.engine.whosTurn())
      .then(next_player_turn => {
        if (this.engine.isGameOver()) {
          // FIXME
          //   right now a bunch of logic is baked into the fcking slack connector class
          let game_results = this.engine.gameResults();

          let deck = this.engine.game_state.getDeck();
          let player_hands = getPlayerHands(this.engine, game_results.winner);

          return this.output.notifyStayThenGameIsOver(game_results, player_hands, deck);
        } else {
  ...
  myCards() {
    ...
    this.output.privateSend(my_hand);
  }
}
