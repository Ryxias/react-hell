...
\`--- !21 game DEBUG INFORMATION ---\``
  ...
  notifyGameBegin(player_hands, deck) {
    return Promise.resolve(() => {
      ...
      .delay(1000)
      .then(() => {
        return this.renderGameInfo(player_hands, deck, false);
      });
  }
...
`
  ...
  notifyDrawThenMoveToNextTurn(card_value, next_player_turn) {
    return Promise
      .resolve(() => this.notifyDrawCard(card_value))
      .delay(500)
  ...
  notifyStayThenMoveToNextTurn(next_player_turn) {
    return Promise.resolve(() => this.send('You stay'))
      .delay(500)
      .then(() => this.notifyEndOfTurn())
      .delay(500)
      .then(() => this.notifyPlayersTurn(next_player_turn));
  ...
  notifyStayThenGameIsOver(game_results, player_hands, deck) {
    return Promise.resolve(() => this.send('You stay'))
      .delay(500)
...
`
  ...
  send(message) {
    return this.public_output(message);
  }

  privateSend(message) {
    return this.private_output(message);
  }
